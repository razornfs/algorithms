...
public class TreePrinter {
    ...
    public static <T extends Comparable<T>> void printNode(MyNode<T> root) {
        int maxLevel = TreePrinter.maxLevel(root);

        printNodeInternal(Collections.singletonList(root), 1, maxLevel);
    }

    private static <T extends Comparable<T>> void printNodeInternal(List<MyNode<T>> nodes, int level, int maxLevel) {
        ...
        for (MyNode<T> node : nodes) {
            ...
            TreePrinter.printWhitespaces(betweenSpaces - (node == null ? 0 : sizeOf(node.getValue()) / 2));
        }
        System.out.println("");
    ...
    private static <T extends Comparable<T>> int sizeOf(T element) {
    ...
    private static <T extends Comparable<T>> int maxLevel(MyNode<T> node) {
        if (node == null)
            return 0;
